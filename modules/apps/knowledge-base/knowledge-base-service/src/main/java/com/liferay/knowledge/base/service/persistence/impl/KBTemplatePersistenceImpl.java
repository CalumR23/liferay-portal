...
public class KBTemplatePersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			ArgumentsResolver.class, new KBTemplateModelArgumentsResolver(),
	...
	private static class KBTemplateModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(kbTemplateModelImpl, columnNames, original);
			}
			...
			if (finderPathColumnBitmask == null) {
				...
				for (String columnName : columnNames) {
					finderPathColumnBitmask |=
						kbTemplateModelImpl.getColumnBitmask(columnName);
				}
		...
		private Object[] _getValue(
			KBTemplateModelImpl kbTemplateModelImpl, String[] columnNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = kbTemplateModelImpl.getColumnOriginalValue(
						columnName);
				}
				else {
					arguments[i] = kbTemplateModelImpl.getColumnValue(
						columnName);
				}
			}
